...
public class CollisionListener implements ContactListener {
    ...
    public void postSolve(Contact c, ContactImpulse ci) {   
        if(c.getFixtureA().getBody().getUserData() instanceof LivingEntity) {
            //LivingEntity e = (LivingEntity) c.getFixtureA().getBody().getUserData();
            
            //int magnitude = (int) ci.normalImpulses[0];

            //if(magnitude > DAMAGE_THRESHOLD) e.damage(magnitude / DAMAGE_THRESHOLD);
        }
        
        if(c.getFixtureB().getBody().getUserData() instanceof LivingEntity) {
           // LivingEntity e = (LivingEntity) c.getFixtureB().getBody().getUserData();
